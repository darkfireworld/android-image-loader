...
final class DiskLruCache implements Closeable {
    ...
    private DiskLruCache(File directory, int appVersion, int valueCount, long maxSize, int maxFileCount, ExecutorService executor) {
        if (executor == null) {
            executorService = new ThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
        } else {
     ...
     * @param executor     thread pool ,maybe null
     * @throws IOException if reading or writing the cache directory fails
     */
    public static DiskLruCache open(File directory, int appVersion, int valueCount, long maxSize, int maxFileCount, ExecutorService executor)
            throws IOException {
        if (maxSize <= 0) {
        ...
        DiskLruCache cache = new DiskLruCache(directory, appVersion, valueCount, maxSize, maxFileCount, executor);
        ...
        directory.mkdirs();
        cache = new DiskLruCache(directory, appVersion, valueCount, maxSize, maxFileCount, executor);
